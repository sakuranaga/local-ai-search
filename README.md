...
OS の検索は遅い。ノイズだらけ。本当に必要なファイルが見つからない。LAS はこのストレスを解決するために作りました。

全文検索とベクトル検索（意味検索）を組み合わせて、キーワードが一致しなくても内容が近いファイルを見つけ出します。PDF・画像も OCR で自動テキスト化して検索対象に。データはローカルから一切外に出ません。
