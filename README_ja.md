...
**NAS の超進化系。エンタープライズ対応のセルフホスト型ファイル基盤に、ハイブリッド AI 検索とローカル LLM エージェントを搭載。完全オンプレ・ゼロクラウド。**

Web UI だけで完結するファイルサーバー。ドラッグ & ドロップでアップロード、フォルダ整理、権限制御、バージョン管理、共同編集、外部共有 — ブラウザから全て操作できます。全文検索 + ベクトル検索（意味検索）で、キーワードが正確に一致しなくても内容が近いファイルを発見。AI エージェントが自律的に文書を探索・要約。PDF・画像は OCR で自動テキスト化。すべて LAN 内で完結し、クラウドへのデータ送信はゼロ。SMB ネットワークドライブにも対応（実験的）。
...
LAS をネットワークドライブとしてマウントします。**FUSE + Samba ハイブリッドアーキテクチャ**: FUSE が DB ベースの読み取り専用仮想ビュー (フォルダ構造 + 権限フィルタリング) を提供し、書き込みはステージングディレクトリ上で純粋な POSIX 操作として実行されます。バックグラウンドワーカーがステージングファイルを永続ストレージと検索インデックスに同期します。

- **SMB/CIFS ネットワーク共有** — Windows/macOS/Linux からネットワークドライブとしてマウント
- **FUSE 仮想ビュー** — LAS のフォルダ/ドキュメント階層を権限ベースの表示制御付きでマッピング (owner/group/others)。ダウンロード禁止ファイルは非表示
- **ステージング書き込み** — Finder/Explorer から直接ファイルの作成・編集・リネーム・削除が可能。全ての書き込み操作はステージングディレクトリ上の POSIX 操作 — あらゆるエディタの保存パターン (Office, Typora, テキストエディット等) に対応
- **バックグラウンド同期** — ステージングファイルは自動的に永続 UUID ストレージに移動、DB に登録、検索インデックスにキューイング (1 秒間隔)
- **LAS ユーザー認証** — LAS のユーザー名/パスワードで接続。書き込み権限は owner/group/others 単位で適用
- **双方向キャッシュ同期** — Redis pub/sub で FUSE メタデータキャッシュと Web UI の同期を維持
...
| Cache | Valkey 8 (Redis 互換、BSD-3-Clause) |
...
journalctl --user -u ocr-server -f      # ログ
```

詳細は [ocr-server/README.md](ocr-server/README.md) を参照。
...
**制約事項:**
- Web UI からファイルを追加した場合、Finder のフォルダ内容は自動更新されません。Cmd+R でリフレッシュするか、フォルダを開き直してください。
- macOS ターミナルの `ls` は "Operation not permitted" と表示される場合があります（TCC 制限）。Finder を使用してください。
- 大きなファイル (10MB 超) はアップロード後、開けるようになるまで数秒かかる場合があります（ステージング → 永続ストレージへの移動中）。
- SMB 接続前に、LAS Web UI に一度ログインする必要があります（パスワード同期のため）。
...
`scripts/backup-s3.sh` で PostgreSQL ダンプ + アップロードファイルを S3 互換ストレージにバックアップ:
...
バックアップ内容:
- **DB ダンプ** — `pg_dump` で全テーブルをダンプ、gzip 圧縮して S3 にアップロード
- **アップロードファイル** — `data/uploads/` を S3 に同期（`aws s3 sync`）

設定は `scripts/backup-s3.sh` 内の変数（`ENDPOINT`, `BUCKET`, `PROFILE`）を環境に合わせて変更してください。AWS CLI プロファイルの事前設定が必要です。

### 連携

| 連携先 | ディレクトリ | 説明 |
|--------|-------------|------|
| Notion | `integration/notion/` | Notion ワークスペースからドキュメントを同期 |
| Discord | `discord-las-bot/` | Discord ボットからドキュメント検索・登録 |

いずれも `POST /api/ingest/content` API を利用したオプション連携です。
...
LAS が依存するライブラリ・ツールのライセンスは以下の通りです。全て AGPL-3.0 と互換性があります。

| カテゴリ | ライブラリ | ライセンス | 備考 |
|---------|-----------|-----------|------|
| Web フレームワーク | FastAPI, Uvicorn, Gunicorn | MIT / BSD | |
| データベース | SQLAlchemy, asyncpg, pgvector, pg_bigm | MIT / Apache-2.0 / PostgreSQL | |
| キャッシュ | Valkey | BSD-3-Clause | Redis 互換 OSS フォーク |
| PDF 解析 | PyMuPDF | AGPL-3.0 | 同一ライセンスファミリー |
| OCR | surya-ocr | GPL-3.0+ | AGPL-3.0 と互換 |
| ウイルススキャン | ClamAV | GPL-2.0 | 別プロセス実行 |
| ドキュメント変換 | LibreOffice | MPL-2.0 | 別プロセス実行 |
| フロントエンド | React, Vite, Tailwind CSS | MIT | |
| エディタ | BlockNote | MPL-2.0 | §3.3 で AGPL 互換 |
| 共同編集 | Yjs, y-websocket | MIT | |
| 動画 | video.js | Apache-2.0 | v7.0+ |
| LLM / Embedding | llama.cpp | MIT | モデルウェイトは別ライセンス |

> **注意**: LLM / Embedding / OCR の**モデルウェイト**にはそれぞれ独自のライセンスがあります（例: Meta Llama Community License, Mistral Apache-2.0 等）。利用するモデルのライセンスを個別に確認してください。
