...
### 概要
フォルダをドラッグ&ドロップまたは選択してアップロードすると、フォルダ階層を自動作成してファイルを配置する機能。Google Drive 等で一般的な操作。

### 要件
- フォルダをドラッグ&ドロップ or `<input webkitdirectory>` で選択
- フォルダ内のファイルを再帰的に取得し、相対パスを保持
- LAS 側でフォルダ階層を自動作成（存在しなければ作成）
- 各ファイルを tus 経由で対応するフォルダにアップロード
- 進捗表示（全体のファイル数 / 完了数）
...
営業資料/
  ├── 見積書/
  │   ├── A社.pdf    → LASフォルダ「営業資料 > 見積書」に配置
  │   └── B社.pdf
  └── 提案書.pptx    → LASフォルダ「営業資料」に配置
```

### 大量ファイル対策

数万件のファイルを含むフォルダのアップロードに対応するための設計。

#### 問題点
- ブラウザメモリ: 数万の File オブジェクトを同時に保持するとメモリ溢れる
- 同時接続: 数万の tus アップロードを一斉開始できない
- UI: ファイルごとにトーストを出すと画面が埋まる
- サーバー負荷: テキスト抽出・ベクトル化の同時処理で崩壊する
...
┌──────────────────────────────────────────────────────────┐
│ フォルダアップロード: 営業資料/                            │
│ ████████████░░░░░░░░░░░░░░  150 / 10,000 件 (3.2GB / 15GB) │
│ ├ アップロード中: A社見積書.pdf (45%)                      │
│ ├ アップロード中: B社提案書.pptx (12%)                     │
│ └ アップロード中: 議事録_Q3.md (89%)                       │
│                                      [一括中止]           │
└──────────────────────────────────────────────────────────┘
```

- **同時アップロード数**: 3〜5 件に制限、残りはキューで待機
- **バッチ進捗 UI**: 個別トーストではなく、1つの進捗パネルで全体の件数・サイズ・現在処理中のファイルを表示
- **一括中止**: バッチ全体をキャンセルするボタン。実行中のアップロードを abort し、キュー内の未開始分は破棄
- **エラースキップ**: 1 ファイル失敗しても残りは続行。完了時にエラーファイル一覧を表示
- **メモリ管理**: ファイル一覧は配列で保持するが、File オブジェクトへの参照はアップロード完了後に解放
...
1. フォルダドロップ / 選択
   ↓
2. FileSystemDirectoryEntry を再帰走査 → ファイル一覧 + 相対パスを取得
   ↓
3. フォルダ階層を一括作成（POST /api/folders/bulk）
   → パス文字列の配列を送信、各フォルダの ID を返却
   → 既存フォルダと同名ならそれを再利用
   ↓
4. アップロードキュー開始
   → 同時 3〜5 件の tus アップロード
   → 各ファイルの metadata に folder_id を付与
   → 完了したら次のキューを開始
   ↓
5. 全件完了 → サマリー表示
   → 成功: N 件、失敗: M 件（失敗一覧を展開表示）
```
...
POST /api/folders/bulk
  body: { paths: ["営業資料", "営業資料/見積書", "営業資料/提案書"], parent_id?: string }
  response: { folders: [{ path: "営業資料", id: "uuid" }, { path: "営業資料/見積書", id: "uuid" }, ...] }
```

### 実装方針

#### フロントエンド
- ドロップイベントで `DataTransferItem.webkitGetAsEntry()` → `FileSystemDirectoryEntry` を再帰走査
- ファイル選択は `<input type="file" webkitdirectory>` で対応
- 各ファイルの相対パスからフォルダ一括作成 API を呼び出し、フォルダ ID マップを取得
- アップロードキュー（同時 3〜5 件）で tus アップロード実行
- tus アップロード時の metadata にフォルダ ID を付与
- バッチ進捗パネルで全体の進捗を表示

#### バックエンド
- フォルダ一括作成エンドポイント: パス文字列の配列から階層を再帰作成し、各レベルのフォルダ ID を返す
- 既存フォルダと重複しない（同名フォルダがあればそれを使用）
...
| 3 | フォルダごとアップロード | UX改善。既存インフラで実現可能 |
