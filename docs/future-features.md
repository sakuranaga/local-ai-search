# 将来機能ロードマップ — 商用NASを超えるファイル管理システム

## ビジョン

AI搭載のローカルファイル管理システムとして、Synology/QNAPなどの商用NASが持つファイル管理機能を網羅しつつ、AI検索・エージェントという独自の強みで差別化する。
...
## 現在のシステムが既に持つ強み（商用NASにない機能）

| 機能 | Synology/QNAP | LAS |
|------|--------------|-----|
| AI エージェント検索 | ✗ | ✓ ツール呼び出しで自律的に文書を探索・回答 |
| ベクトル検索 | ✗ | ✓ 意味的な類似検索 |
| 全文検索 + RRF統合 | △ 基本的 | ✓ 全文+ベクトルのハイブリッド |
| OCR → 検索可能化 | △ アドオン | ✓ 組み込み |
| ドキュメント自動要約 | ✗ | ✓ LLMで自動生成 |
| テキスト編集（WYSIWYG） | ✗ | ✓ OverType エディタ |
| API キーによる外部連携 | △ 限定的 | ✓ フォルダスコープ付き |
| tus レジューマブルアップロード | ✗ | ✓（実装予定） |
| AI フォルダ自動整理 | ✗ | ✓（実装予定）ベクトル+LLMハイブリッド |
