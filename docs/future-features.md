...
現状、複数ユーザーが同時にテキスト編集やファイル上書きを行うと後勝ち（last-write-wins）になる。バージョン管理で変更は消えないが、先に保存した人の変更が無言で上書きされる。

### 段階的な実装アプローチ

#### Phase 1: 楽観的ロック（競合検出）

保存時にクライアントが「読み込み時のバージョン番号」を送信し、DB のバージョンと不一致なら 409 Conflict を返す。

- `PATCH /documents/{id}` に `expected_version` パラメータを追加
- バージョン不一致時は「他のユーザーが変更しました。リロードしてください」とUIで表示
- 最も低コストで後勝ち問題を解消できる

#### Phase 2: 悲観的ロック（編集ロック）

編集開始時にロックを取得し、他のユーザーには「○○さんが編集中」と表示する。
...
- テキスト編集タブを開いたときに自動ロック取得
- タブを閉じる・離脱時に自動解放
- タイムアウト（例: 30分）で自動解放（ブラウザクラッシュ対策）
- WebSocket で他ユーザーにロック状態をリアルタイム通知

```
POST /api/documents/{id}/lock       — ロック取得
DELETE /api/documents/{id}/lock     — ロック解放
```

#### Phase 3: リアルタイム共同編集（CRDT）

Google Docs のような同時編集。Yjs (CRDT ライブラリ) + WebSocket で実現。

- **Yjs** — CRDT ベースのリアルタイム共同編集ライブラリ
- **y-websocket** — Yjs の WebSocket プロバイダ
- **y-prosemirror** or **y-codemirror** — エディタバインディング
- 現在の OverType エディタを ProseMirror or CodeMirror ベースに置き換え、Yjs で同期
- カーソル位置・選択範囲のリアルタイム共有（awareness protocol）
- オフライン編集 → 再接続時の自動マージ

### 実装優先度

Phase 1 は低コスト（バックエンド数行 + フロントエンド数十行）で効果が高いため即座に実装可能。Phase 2 は中規模の組織で十分。Phase 3 はプロダクトとしての差別化になるが実装コストが大きい。
...
ドキュメントの公開・共有・変更に対して上長やチームリーダーの承認を必要とするワークフロー機能。企業利用では「誰が確認したか」の記録が求められる場面が多い。

### 機能パターン

#### A. 公開承認（基本）

ドキュメントのステータスを「下書き → 承認待ち → 承認済み」で管理。

- アップロード・編集されたドキュメントは「下書き」状態
- 作成者が「承認申請」ボタンで承認者に申請
- 承認者が「承認」or「差し戻し」（コメント付き）
- 承認済みドキュメントのみ検索対象に含める（オプション）

#### B. 共有承認

外部共有リンク作成時に承認を必須にする。

- 共有リンク作成 → 承認待ち状態（リンクは無効）
- 承認者が承認 → リンク有効化
- 機密ファイルの外部流出を防止

#### C. 多段階承認

複数の承認者による順次承認（課長 → 部長など）。

- 承認ルートをテンプレートとして定義
- フォルダ単位でデフォルト承認ルートを設定可能
...
CREATE TABLE approval_routes (
    ...
    name VARCHAR(200) NOT NULL,           -- "営業部 共有承認" 等
    approval_type VARCHAR(20) NOT NULL,   -- publish, share, edit
    steps JSONB NOT NULL,                 -- [{"order": 1, "approver_id": "..."}, ...]
...
CREATE TABLE approval_requests (
    ...
    approval_type VARCHAR(20) NOT NULL,   -- publish, share
...
- ドキュメント詳細モーダルに承認ステータスバッジ（下書き / 承認待ち / 承認済み / 差し戻し）
- 承認者のダッシュボード（未処理の承認申請一覧）
- 承認履歴タイムライン（誰がいつ承認/差し戻したか）
- メール通知連携（承認申請時・承認/差し戻し時）

### エンドポイント

```
POST   /api/documents/{id}/approval/request    — 承認申請
POST   /api/documents/{id}/approval/approve    — 承認
POST   /api/documents/{id}/approval/reject     — 差し戻し（コメント付き）
GET    /api/approval/pending                   — 未処理の承認申請一覧
GET    /api/documents/{id}/approval/history     — 承認履歴
GET    /api/approval/routes                    — 承認ルート一覧 (admin)
POST   /api/approval/routes                    — 承認ルート作成 (admin)
```
...
| 2 | 同時編集制御 | データ整合性。Phase 1（楽観的ロック）は即実装可能 |
| 3 | 承認ワークフロー | 企業利用で必須。共有承認だけでもセキュリティ面で価値大 |
| 4 | SSO / エンタープライズ認証 | 企業導入時の必須要件になりうる |
