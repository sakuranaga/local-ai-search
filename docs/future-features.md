...
- **ノート（BlockNote）**: Yjs によるリアルタイム共同編集をノート機能と同時に実装 → [design-notes.md](design-notes.md) 参照
- **検索テキスト・ファイル上書き・メタ情報**: 早い者勝ち（last-write-wins）のまま。バージョン管理でカバー
...
Notion 風ブロックエディタ（BlockNote）によるナレッジ作成機能。Yjs リアルタイム共同編集 + AI コマンド付き。

**詳細設計:** [docs/design-notes.md](design-notes.md)
...
| 2 | ノート / Wiki + Yjs 共同編集 | ナレッジ作成 + リアルタイム共同編集。BlockNote + y-websocket |
| 3 | 同時編集制御（検索テキスト等） | 楽観的ロック or 悲観的ロック。ノート以外の競合対策 |
| 4 | 承認ワークフロー | 企業利用で必須。共有承認だけでもセキュリティ面で価値大 |
| 5 | SSO / エンタープライズ認証 | 企業導入時の必須要件になりうる |
| 6 | ストレージクォータ | マルチユーザー運用で必要 |
| 7 | S3連携 | 大規模運用・クラウド移行時に必要 |
