...
### 概要
システムイベント（ログイン、ファイル追加・更新・削除）発生時にメールで通知する機能。

### 通知対象アクション

| アクション | トリガー | メール内容 |
|-----------|---------|-----------|
| ログイン | ユーザーログイン成功時 | ログインユーザー名、日時、IPアドレス |
| 新規追加 | ドキュメントアップロード完了時 | ファイル名、アップロード者、フォルダ |
| 更新 | ドキュメント編集・上書き時 | ファイル名、更新者、変更内容の概要 |
| 削除 | ドキュメント削除時（ゴミ箱移動） | ファイル名、削除者 |

**バルク操作**: 一括削除・一括移動等は1通にまとめる（「5件のファイルが削除されました」形式）

### データモデル

```sql
-- メールプロバイダ設定（SystemSetting に保存）
-- mail_provider: "smtp" | "sendgrid" | "resend" | "ses"
...
CREATE TABLE mail_recipients (
    ...
    on_login BOOLEAN DEFAULT FALSE,
    on_create BOOLEAN DEFAULT FALSE,
    on_update BOOLEAN DEFAULT FALSE,
...
プロバイダ切替で入力フォームが動的に変化:
- **SMTP**: ホスト、ポート、ユーザー名、パスワード、TLS
- **SendGrid**: API キー
- **Resend**: API キー
- **SES**: リージョン、アクセスキー、シークレットキー

「テスト送信」→ 管理者のメールアドレスにテストメールを送信、成功/失敗をトースト表示
...
│ 通知先                                      [+ 追加]        │
├──────────────────────────────────────────────────────────────┤
│ メールアドレス          ログイン  新規  更新  削除            │
│ shuzan@gmail.com          ☑️     ☑️    ☑️    ☑️    [🗑]    │
│ tanaka@example.com        ☐     ☑️    ☑️    ☐     [🗑]    │
│ security@example.com      ☑️     ☐    ☐     ☐     [🗑]    │
└──────────────────────────────────────────────────────────────┘
```

- チェックボックスは即時反映（PATCH リクエスト、トースト不要）
- 「+ 追加」でメールアドレス入力ダイアログ
- 🗑 で削除（確認ダイアログ）
...
```
GET    /api/admin/mail/config           — メール設定取得
PUT    /api/admin/mail/config           — メール設定保存
POST   /api/admin/mail/test             — テストメール送信
GET    /api/admin/mail/recipients       — 通知先一覧
POST   /api/admin/mail/recipients       — 通知先追加
PATCH  /api/admin/mail/recipients/{id}  — 通知先更新（チェックボックス即時反映）
DELETE /api/admin/mail/recipients/{id}  — 通知先削除
```

### 実装方針

- メール送信は `asyncio.create_task` + `run_in_executor` で別スレッドバックグラウンド実行。SMTP 接続遅延・タイムアウト等がシステム操作を一切ブロックしない
- 送信失敗時はリトライ（最大3回、指数バックオフ）、失敗ログを audit_log に記録。送信エラーがユーザー操作に影響しない（fire-and-forget）
- メールテンプレートは HTML（Jinja2）、シンプルなデザイン
- バルク操作は操作完了後に1通にまとめて送信
