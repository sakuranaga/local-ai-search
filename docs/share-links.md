...
### バックグラウンド転送
現在、共有リンク作成時にファイル転送を同期的に実行している。
大きなファイル（1GB+）の場合、ユーザーが長時間待たされる。

改善案:
1. DBにレコード作成（status: "transferring"）→ ダイアログを即閉じる
2. バックグラウンドタスクでShare Serverにファイル転送
3. 完了 → status: "active" + トースト通知
4. 失敗 → status: "failed" + エラー通知
5. 共有タブで転送中ステータスを表示
